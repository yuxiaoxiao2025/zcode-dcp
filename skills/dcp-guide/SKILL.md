...
| 命令 | 用途 |
|---|---|
| `/dcp-stats` | 真实发送/节省 token、各策略命中次数、节省率（saved/(sent+saved)，非上游缓存命中率） |
| `/dcp-context` | 上下文构成估算 |
| `/dcp-compress [focus]` | 手动触发一次压缩 |
| `/dcp-sweep [N]` | 裁剪最近 N 条工具调用（无参则裁上一 user 后的全部）；延迟应用——实际生效在该会话下一请求 |
| `/dcp-manual on\|off` | 手动模式（禁模型自主压缩） |
| `/dcp-decompress` / `/dcp-recompress` | 无参=列出可用压缩块（`b<N> (~T tokens) - topic`）；带 `blockId=<n>`=按块号加入排除表（下一请求原文恢复）；`/dcp-recompress`=清排除表+关 manualMode（全部恢复） |
| `/dcp-setup` | 安装配置指引 |
| `/dcp-help` | 命令总览 |
...
| 真实用量/缓存命中率 | **裁剪后真实消耗**（上游 usage 逐字节透传）——真实省钱看这里。注意：/dcp-stats 里的 "Savings rate" 是本地节省率，与这里的上游缓存命中率是两个指标 |
